...
export default function StudentDashboardPage() {
    ...
    return (
        <PageSection title="Calendrier etudiant" subtitle="Vue dediee aux evenements de cours de l etudiant connecte.">
            ...
            {isLoading ? (
                <p className="text-ink-800">Chargement du calendrier...</p>
            ) : null}
                ...
                <div className="overflow-x-auto">
                    <div className="grid min-w-[960px] grid-cols-5 gap-3">
                        {weekCalendar.map((day) => (
                            <section key={day.key} className="rounded-xl border border-ink-500/30 bg-white/60 p-4">
                                <p className="text-sm font-semibold text-black">{day.label}</p>
                                <p className="text-xs text-ink-700">{day.date.toLocaleDateString()}</p>

                                {day.events.length === 0 ? (
                                    <p className="mt-4 text-xs text-ink-600">Aucun evenement</p>
                                ) : (
                                    <ul className="mt-3 space-y-2">
                                        {day.events.map((eventItem) => (
                                            <li key={eventItem.id} className="rounded-md border border-white/10 bg-white/5 p-2">
                                                <p className="text-sm font-medium text-black">{eventItem.courseName}</p>
                                                <p className="text-xs text-accent-600">{eventItem.className}</p>
                                                <p className="text-xs text-ink-700">{eventItem.professorPseudo}</p>
